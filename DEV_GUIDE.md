...
## 八、二开（自定义开发）与上游同步最佳实践

目标：既能落地自己的定制改动，又能在上游更新时低成本合并。以下规矩应该从第一行二开代码开始就遵守，越早养成习惯，未来冲突越少。

### 1. 二开代码物理隔离，别改上游共享文件

- 新功能优先落在新文件/新包里（例如 `internal/service/mycompany_xxx.go`），而不是往上游现有的 `gateway_*.go`、`account_*.go` 里直接加分支逻辑。
- 如果必须改共享逻辑（比如加一个 hook 点），尽量做成"在文件末尾/独立函数里加一小段可插拔的调用"，而不是散在函数中间大改——即使产生冲突，diff 也容易看懂、容易重新应用。
- 前端同理：自定义组件/页面放独立目录（如 `frontend/src/custom/`），不要直接改上游共享组件的内部实现，能加 props/slot 就不要复制粘贴改源码。

### 2. 配置类改动用 feature flag / 新增配置项，而不是改默认行为

- 参考上游已有的 `RUN_MODE=simple` 先例：新增一个环境变量/配置开关来控制定制逻辑是否生效，上游默认路径保持不动。

### 3. 数据库 migration 永远新增文件，不改老文件

- 迁移一旦在任意环境执行过就不可变（校验和记录在 `schema_migrations` 表中）。二开要加表/加字段，按顺序号新建 `NNN_xxx.sql`，不要改上游已有的迁移文件。
- 涉及 `CREATE/DROP INDEX CONCURRENTLY` 的文件按约定加 `_notx.sql` 后缀。

### 4. Go module 路径、包名保持 `github.com/Wei-Shaw/sub2api` 不变

- 一旦改名，几乎每个上游文件的 import 都会产生合并冲突，得不偿失。

### 5. 固化同步流程，定期同步

- 主分支 `main` 用 `git merge upstream/main`（保留清晰的合并点，出问题好定位是哪次同步引入的）；自己的 feature 分支开发时可以 `rebase` 保持线性，避免团队里两种方式混用。
- 定期同步（比如每周，或上游有大版本发布时），不要攒几个月不同步——冲突量随时间基本是指数增长，攒得越久一次性解决成本越高。
- 每次同步后，重新跑一遍 `go generate ./ent`、`go generate ./cmd/server`、前端 `pnpm run build`，防止生成代码和上游手写代码脱节，并补充执行「坑 9」「坑 6」里提到的检查。

### 6. 提交历史把"定制改动"和"跟随上游"分开

- 二开的 commit message 加统一前缀（例如 `feat(custom): xxx`），方便日后 `git log --grep` 快速筛出哪些是自己的改动、哪些是同步上游带来的；出现冲突时也好判断这段代码是不是自己加的。
